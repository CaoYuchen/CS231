...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  dnext_c_h=dnext_h*o*dtanh_c
  dprev_c=dnext_c_h*f+dnext_c*f
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  for t in xrange(T):
    next_h, next_c, cache_ = lstm_step_forward(x[:,t,:], next_h, next_c, Wx, Wh, b)
    h[:,t,:]=next_h
    cache.append(cache_)
...
def lstm_backward(dh, cache):
  ...
  for t in xrange(T-1,-1,-1):
    dnext_h=dh[:,t,:]+dprev_h
    dx_step, dprev_h, dprev_c, dWx_step, dWh_step, db_step = lstm_step_backward(dnext_h, dprev_c, cache[t])
    dx[:,t,:]+=dx_step
    dWx+=dWx_step
    dWh+=dWh_step
    db+=db_step
